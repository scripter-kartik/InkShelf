    ...
    return (<div className="bg-gray-100 min-h-screen w-[calc(100vw - 16px)] overflow-x-hidden">
      ...
      <div className="p-4 bg-green-400 rounded-md m-2 md:m-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {loading
            ? Array.from({ length: 12 }).map((_, i) => (<div key={i} className="bg-white rounded shadow-md p-3 animate-pulse">
            ...
            : books.slice(0, visibleCount).map((book) => (<div key={book.key} className="bg-white rounded shadow-md p-3 hover:shadow-xl transition-shadow">
        ...
        {!loading && visibleCount < books.length && (<div className="flex justify-center mt-6">
            <button onClick={handleLoadMore} className="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">
              Show More
            </button>
          </div>)}
      </div>
