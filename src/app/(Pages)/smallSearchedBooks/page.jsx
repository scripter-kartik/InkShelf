...
export default function SmallSearchedBooks() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (!browseRef.current?.contains(event.target)) setBrowseOpen(false);
      if (!buttonRef.current?.contains(event.target)) setIsActive(false);
    };
    ...
    <div>
      ...
      >
        <div className="relative" ref={suggestionRef}>
          <input
            className="border-2 font-lato text-md rounded px-3 sm:w-96 h-10 outline-none text-gray-500"
            type="text"
            placeholder="search"
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && query.trim()) {
                router.push(
                  `/searchedBooks?q=${encodeURIComponent(query.trim())}`
                );
                setShowSuggestions(false);
              }
            }}
        ...
        <div className="flex items-center sm:gap-7 gap-4">
          <div className="relative" ref={browseRef}>
            <div
              ...
              className="flex gap-1 font-winky items-center cursor-pointer"
            >
              <button>browse</button>
              <img className="w-2 h-2" src="/downArrow.png" alt="Dropdown" />
            </div>
            {browseOpen && (
              <div className="absolute top-8 left-[-170px] mt-2 w-56 max-h-96 overflow-y-auto bg-white rounded shadow-md border border-gray-300 z-50">
          ...
          <img
            className="w-7 h-7 mr-3 sm:hidden cursor-pointer"
      ...
      {isActive && (
        <div
          ref={buttonRef}
          className="flex flex-col gap-5 items-center w-32 h-48 p-3 rounded border-2 bg-white border-gray-300 absolute right-0 top-16 text-xl font-lato z-40"
        >
